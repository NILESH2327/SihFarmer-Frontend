...
const WeatherCard = ({ Weather, setWeather }) => {
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        // Use your existing geo logic to get city name
    ...
    <div className="bg-white rounded-2xl shadow-md border border-green-100 p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative w-64 sm:w-80">
            <input
              ...
              className="w-full border border-gray-300 rounded-lg pl-4 pr-12 py-2.5 shadow-sm focus:ring-2 focus:ring-blue-500 focus:outline-none text-sm"
            />
            <button
              onClick={handleSearch}
              className="absolute right-0 top-1/2 -translate-y-1/2 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-md flex items-center justify-center h-full transition"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {Weather.forecast.forecastday.map((day, index) => (
          <div
            key={index}
            className="text-center p-4 bg-gray-50 rounded-lg shadow-sm border border-gray-100"
          >
